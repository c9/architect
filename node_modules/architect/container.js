...
function listen(socketPath, callback) {
  ...
  function mapService(serviceName, containerName) {
    ...
    Object.keys(pendingPlugins).forEach(function (key) {
      ...
      if (checkDependencies(item.options.dependencies)) {
  ...
  function startPlugin(name, options, callback) {
    ...
    if (!checkDependencies(options.dependencies)) {
      pendingPlugins[name] = {options: options, callback: callback};
      return;
    }

    var moduleName = options.module || name;
    var modulePath = require.resolve(moduleName[0] === "." ? path.resolve(config.base, moduleName) : moduleName);
    ...
    options.dependencies && options.dependencies.forEach(function (dependency) {
      ...
      // TODO: create service proxies for peer-to-peer connections
